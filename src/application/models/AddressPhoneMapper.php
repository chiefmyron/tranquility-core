...
use \Tranquility\Enum\Address\SubtypePhone as EnumPhoneAddressType;


class AddressPhoneMapper extends AddressMapper {
    ...
    /**
     * Retrieves a list of phone addresses associated with the specified parent ID
     * 
     * @param int     $parentId
    ...
    public function getAddressList($parentId, $resultsPerPage, $start, $verbose = false, $filter = null) {
        $results = $this->_getAddresses($parentId, EnumAddressType::Phone, $resultsPerPage, $start, $filter);
    ...
    /**
     * Creates a new telephone address record
     * 
     * @param array   $values
     * @param boolean $verbose
     * @return \Tranquility\Response
    ...
    public function createAddress($values, $verbose) {
        $this->_log->debug('Start of AddressesPhoneMapper::createAddress() method', $values);
        ...
        try {
            ...
            $id = $this->_createEntityRecord(EnumEntityType::Address, EnumAddressType::Phone);
            ...
            if ($primaryContact == 1) {
                $this->_log->debug('Changing primary contact flag for telphone address');
            ...
            $query  = "INSERT INTO tql_entity_addresses_phone (id, addressType, addressText, primaryContact, transactionId) \n";
            $query .= "VALUES (:id, :addressType, :addressText, :primaryContact, :transactionId)";
            $inserts = array(
                'id'             => $id,
        ...
        $response = $this->getAddressById($parentId, $id, EnumAddressType::Phone, $verbose);
    ...
    /**
     * Updates an existing telephone address record
     * 
     * @param array   $values
     * @param boolean $verbose
     * @return \Tranquility\Response
    ...
    public function updateAddress($values, $verbose) {
        $this->_log->debug('Start of AddressesPhoneMapper::updateAddress() method', $values);
        ...
        try {
            ...
            // If this is the new primary contact, remove flag from previous primary contact
            if ($primaryContact == 1) {
                $this->_log->debug('Changing primary contact flag for telephone address');
                $result = $this->_removePrimaryContactFlag($parentId, $transactionId);
                if ($result == false) {
                    $this->_db->rollback();
                    $response->setResponseCode(EnumStatusCodes::InternalServerError);
            ...
            $result = $this->_createHistoricalPhoneAddressRecord($id);
            if ($result == false) {
                $this->_db->rollback();
                $response->setResponseCode(EnumStatusCodes::InternalServerError);
            ...
            $query  = "UPDATE tql_entity_addresses_phone \n";
    ...
    /**
     * Logically deletes the telephone address record for the supplied ID
     * 
     * @param array   $values
     * @param boolean $verbose
     * @return \Tranquility\Response
     ...
     * @return boolean
     */
    protected function _createHistoricalPhoneAddressRecord($id) {
        // Copy existing record into historical table
        $query  = "INSERT INTO tql_history_addresses_phone (id, version, addressType, addressText, primaryContact, transactionId) \n";
        $query .= "SELECT a.id, a.version, b.addressType, b.addressText, b.primaryContact, b.transactionId \n";
        $query .= "FROM tql_entity AS a, \n";
        $query .= "     tql_entity_addresses_phone AS b \n";
        $query .= "WHERE a.id = ? \n";
        $query .= "AND a.id = b.id ";
    ...
    /**
     * Removes the 'Primary Contact' flag from any existing telephone addresses
     * 
     * @param  int $parentId
    ...
    protected function _removePrimaryContactFlag($parentId, $transactionId) {
        ...
        $result = $this->_getAddresses($parentId, EnumAddressType::Phone, 1, 1, $filter);
        if ($result == false) {
        ...
        if (count($result) > 1) {
            $this->_log->error('Multiple telephone addresses flagged as primary contact for parent entity '.$parentId);
        }
        ...
        $result = $this->_createHistoricalPhoneAddressRecord($address->id);
        if ($result == false) {
            return $result;
        }
    ...
    /**
     * Validate all inputs for creating or updating a telephone address record
     * 
     * @param array   $inputs
    ...
    public function validateInputFields($inputs, $newRecord = false, $validateAuditTrail = true) {
        ...
        if (!EnumPhoneAddressType::isValidValue($addressType)) {
